...
// TX latency running mean (EMA with alpha = 1/16)
#define TX_LATENCY_EMA_SHIFT 4
static volatile int64_t tx_latency_ema_ns = 0;
static volatile uint32_t tx_latency_sample_count = 0;
...
bool hw_timestamp_find_tx(uint32_t counter_before, int64_t *latency_ns) {
    ...
    *latency_ns = -delta_ns;
    ...
    if (*latency_ns >= 1000 && *latency_ns < 1000000) {
        // Update EMA
        if (tx_latency_sample_count == 0) {
            tx_latency_ema_ns = *latency_ns;
        } else {
            tx_latency_ema_ns += (*latency_ns - tx_latency_ema_ns) >> TX_LATENCY_EMA_SHIFT;
        }
        tx_latency_sample_count++;
        return true;
    }
...
/**
 * Get TX latency running mean in nanoseconds
 */
int64_t hw_timestamp_get_tx_latency_mean_ns(void) {
    return tx_latency_ema_ns;
}
