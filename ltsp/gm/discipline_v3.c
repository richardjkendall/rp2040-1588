...
/* LTSP regression for crystal characterisation */
...
// LTSP regression state
static ltsp_regression_sample_t regression_buf[LTSP_REGRESSION_DEFAULT_WINDOW];
static ltsp_regression_t regression;
static ltsp_pio_ts_t pps_pio_ts;                // 64-bit extended PIO counter for 1PPS
static volatile uint32_t last_1pps_interval_ticks = EXPECTED_TICKS_PER_SECOND;
static volatile bool regression_sample_pending = false;
static volatile int64_t pending_sample_t = 0;
static volatile int64_t pending_sample_e = 0;
...
static void __time_critical_func(discipline_pps_irq_handler)(void) {
    ...
    // LTSP regression: extend PIO counter to 64-bit and queue sample
    ltsp_pio_ts_extend(&pps_pio_ts, counter_at_pps);
    last_1pps_interval_ticks = elapsed_ticks;
    if (!regression_sample_pending) {
        pending_sample_t = (int64_t)ltsp_pio_ts_to_u64(&pps_pio_ts);
        pending_sample_e = phase_error_ticks;
        regression_sample_pending = true;
    }
...
bool discipline_init_v3(void) {
    ...
    // Initialize LTSP regression
    ltsp_regression_init(&regression, regression_buf, LTSP_REGRESSION_DEFAULT_WINDOW);
    ltsp_pio_ts_init(&pps_pio_ts);
    printf("  LTSP regression initialized (window=%d)\n", LTSP_REGRESSION_DEFAULT_WINDOW);
...
void discipline_recompute_regression(void) {
    if (regression_sample_pending) {
        int64_t t = pending_sample_t;
        int64_t e = pending_sample_e;
        regression_sample_pending = false;

        ltsp_regression_add_sample(&regression, t, e);
        if (regression.count >= 2) {
            ltsp_regression_compute(&regression);
        }
    }
}

/**
 * Get LTSP regression results
 */
bool discipline_get_regression(double *a0, float *a1_ppb, float *sigma_ns) {
    if (!regression.result.valid) return false;
    *a0 = regression.result.a0;
    *a1_ppb = regression.result.a1_ppb;
    *sigma_ns = regression.result.sigma_ns;
    return true;
}

/**
 * Re-reference a0 to a given epoch
 */
double discipline_get_a0_at_epoch(int64_t t_epoch) {
    return ltsp_regression_a0_at(&regression, t_epoch);
}

/**
 * Get last 1PPS info for LTSP PDU
 */
void discipline_get_1pps_info(int64_t *last_1pps_counter, uint32_t *last_1pps_interval) {
    *last_1pps_counter = (int64_t)ltsp_pio_ts_to_u64(&pps_pio_ts);
    *last_1pps_interval = last_1pps_interval_ticks;
}
